...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        action = max(gameState.getLegalActions(0), key=lambda x: minimax_search(gameState.generateSuccessor(0, x), 1, 1))
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minimax_search(state, agentIndex, depth, alpha, beta):
            # if in min layer and last ghost
            if agentIndex == state.getNumAgents():
                # if reached max depth, evaluate state
                if depth == self.depth:
                ...
                # otherwise start new max layer with bigger depth
                    ...
                    return minimax_search(state, 0, depth + 1, alpha=-float("inf"), beta=float("inf"))
            # if not min layer and last ghost
            else:
                ...
                if agentIndex == 0: # max player
                    ...
                    for action in actions:
                        value = max(value, minimax_search(state.generateSuccessor(agentIndex, action), agentIndex+1, depth, alpha, beta))
                        if value >= beta:
                            return value
                    ...
                    value = float("inf")
                    for action in actions:
                        value = min(value, minimax_search(state.generateSuccessor(agentIndex, action), agentIndex+1, depth, alpha, beta))
                        if value <= alpha:
                            return value
                        beta = min(beta, value)
                    return value
        ...
        # select the action with the greatest minimax value
        result = max(gameState.getLegalActions(0), key=lambda x: minimax_search(gameState.generateSuccessor(0, x), 1, 1, alpha=-float("inf"), beta=float("inf")))
        ...
        def minimax_search(state, agentIndex, depth):
            ...
            # if not min layer and last ghost
                ...
                if agentIndex == 0: # max player
                    value = -float("inf")
                    for action in actions:
                        value = max(value, minimax_search(state.generateSuccessor(agentIndex, action), agentIndex+1, depth))
                    return value
                else: # min player
                    value = 0
                    for action in actions:
                        value += random.uniform(0, gameState.getNumAgents()-1) * minimax_search(
...
def betterEvaluationFunction(currentGameState):
    ...
    def avoidStanding():
        global standingTimes
        global prevGameState
        if prevGameState is None:
            prevGameState = currentGameState
            return 0
        currentPos = currentGameState.getPacmanPosition()
        prevPos = prevGameState.getPacmanPosition()
        if manhattanDistance(currentPos, prevPos) == 0:
            standingTimes += 1
        if standingTimes >= 2:
            foodList = newFood.asList()
            mostDistantFood = max([manhattanDistance(newPos, food) for food in foodList]) if len(foodList) > 0 else 0
            standingTimes = 0
            return -max(mostDistantFood, 10)
        return 0
    ...
    return -2*closestFood + 30*ghost_distance - 50*len(foodList) + closest_capsule + avoidStanding()
